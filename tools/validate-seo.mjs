...
const sitemapPaths = ["/", "/about/", "/how-to/", "/sources/", "/privacy/", "/accessibility/"];
const failures = [];
...
const locUrls = Array.from(sitemap.matchAll(/<loc>([^<]+)<\/loc>/g), match => match[1]);
if (locUrls.some(url => url.includes("?") || url.includes("/word/") || url.includes("/search"))) {
  fail("Sitemap includes a query, search route, or word-entry route");
}

const robots = readDist("robots.txt");
if (!robots.includes(`Sitemap: ${origin}/sitemap.xml`)) fail("robots.txt does not declare production sitemap");
if (!robots.includes("Disallow: /api/")) fail("robots.txt does not disallow API routes");

const redirects = readDist("_redirects");
if (!redirects.includes("/search / 301")) fail("_redirects does not canonicalize /search");

const headers = readDist("_headers");
if (!headers.includes("X-Robots-Tag: noindex, nofollow")) fail("_headers missing API noindex rule");
...
for (const [path, file] of routeFiles) {
  ...
  if (!html.includes(canonical)) fail(`${file} missing canonical ${origin}${path}`);
  if (!html.includes("<meta name=\"description\"")) fail(`${file} missing meta description`);
  if (!html.includes("<h1")) fail(`${file} missing h1`);
}

const notFound = readDist("404.html");
if (!notFound.includes("noindex,follow")) fail("404 page missing noindex,follow meta robots");
