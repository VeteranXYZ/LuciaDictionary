...
import { buildCoreFormIndex, lookupCoreBase } from "../src/scripts/dictionary.js";
...
const SAMPLE_WORDS = ["address", "activity", "activities", "privacy", "policy", "thoughtful", "supportive", "fun", "lab", "inbox"];
...
const inflectedDuplicateKeys = keys.filter(key => formIndex[key] && formIndex[key] !== key);
const duplicateKeyCount = 0;
const skippedCandidateCount = Math.max(0, TOP_CANDIDATES - keys.filter(key => core[key].rank <= TOP_CANDIDATES).length);

console.log(`high-frequency source word count: ${fileLines(HFV_PATH)}`);
console.log(`top ${TOP_CANDIDATES} candidate count: ${Math.min(TOP_CANDIDATES, fileLines(HFV_PATH))}`);
console.log(`ECDICT source row count: ${Math.max(0, fileLines(ECDICT_PATH) - 1)}`);
...
console.log(`likely inflected-form duplicate count: ${inflectedDuplicateKeys.length}`);
console.log(`required word coverage: ${REQUIRED_WORDS.length - missingRequired.length}/${REQUIRED_WORDS.length}`);
console.log(`missing required words: ${missingRequired.join(", ") || "none"}`);
console.log(`file size of core-lexicon.json: ${formatBytes(fs.statSync(CORE_PATH).size)}`);
...
for (const word of SAMPLE_WORDS) {
  ...
  console.log(`- ${word}: ${entry ? `${resolved.base} => ${entry.cn}` : "missing"}`);
}
