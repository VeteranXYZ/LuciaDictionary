...
export function pickVoice() {
  ...
  const blacklist =
    "Zarvox,Bad,Whisper,Boing,Bells,Trinoids,Cellos,Pipe,Organ,Deranged,Hysterical,Superstar,Wobble,Bubbles,Good News,Jester,Junior,Ralph,Kathy,Fred,Princess,Albert,Bruce,Bahh,Eddy,Flo,Grandma,Grandpa,Reed,Rocko,Sandy,Shelley,Snoop".split(
      ",",
    );
  ...
  for (const pref of prefs) {
    const voice = voices.find(
      (item) => item.name === pref || item.name.indexOf(pref) >= 0,
    );
  ...
  bestVoice =
    voices.find(
      (item) =>
        item.lang === "en-US" && !item.localService && !isBad(item.name),
    ) ||
...
export function speakWordN(word, el) {
  ...
  speechSynthesis.cancel();
  qsa(".word-card.speaking").forEach((card) =>
...
export function clearSentenceHighlight() {
  sentenceTextEl
    ?.querySelectorAll?.(".speaking-word")
    .forEach((item) => item.classList.remove("speaking-word"));
  sentenceHighlightIndex = -1;
}

export function highlightSentenceToken(index) {
  if (!sentenceTokens.length) return;
  sentenceTokens.forEach((token, i) =>
    token.el.classList.toggle("speaking-word", i === index),
  );
...
export function startSentenceSpeech(text, options = {}) {
  ...
  sentenceTokens = sentenceTextEl
    ? renderSpeakableText(sentenceTextEl, options.displayText || text)
    : [];
  sentenceHighlightIndex = -1;
  sentenceBoundarySeen = false;

  speechSynthesis.cancel();
  qsa(".word-card.sentence-active").forEach((card) =>
    card.classList.remove("sentence-active"),
  );
  ...
  const tokens = (text.match(/[a-zA-Z']+/g) || []).map((token) =>
    token.toLowerCase(),
  );
  let lastCard = null;
  ...
  utterance.onboundary = (event) => {
    ...
    const word =
      sentenceTokens[tokenIndex]?.text.toLowerCase() || tokens[tokenIndex];
    if (!word) return;
    const card = cardByWord[word];
    if (lastCard && lastCard !== card)
      lastCard.classList.remove("sentence-active");
    if (card) {
...
function scheduleFallbackHighlight() {
  if (
    ...
    sentenceBoundarySeen ||
    sentenceFallbackTimer ||
    sentenceFallbackDelayTimer
  )
    return;
  sentenceFallbackDelayTimer = setTimeout(() => {
    sentenceFallbackDelayTimer = null;
    if (!sentenceBoundarySeen && sentenceState === "speaking")
